...
import { User, Mail, Phone, MapPin, Upload, CheckCircle2, AlertCircle, Calendar, Globe, Trash2, FileText, ShieldCheck, Eye, RefreshCw, Clock } from 'lucide-react';
...
export default function StaffProfile() {
  ...
  const [contactInfo, setContactInfo] = useState({
    ...
    address: '42 High Street, Flat 4, London, E1 6AN',
  });

  const [availability, setAvailability] = useState({
    days: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
    hours: 'Day Shifts (08:00 - 20:00)',
    roles: ['Healthcare Assistant', 'Senior Healthcare Assistant'],
  });

  const availableDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
  const availableHours = ['Day Shifts (08:00 - 20:00)', 'Night Shifts (20:00 - 08:00)', 'Any Hours'];
  const availableRoles = ['Healthcare Assistant', 'Senior Healthcare Assistant', 'Support Worker', 'Registered Nurse'];
  ...
  const [docs, setDocs] = useState<ComplianceDoc[]>([
    { id: '1', type: 'Right to Work (Passport)', status: 'valid', expiryDate: '2028-10-20', fileName: 'passport_sj.pdf' },
    { id: '2', type: 'Enhanced DBS', status: 'valid', expiryDate: '2025-03-15', fileName: 'dbs_cert_2024.pdf' },
    { id: '3', type: 'Manual Handling Training', status: 'expiring', expiryDate: '2026-04-01', fileName: 'mh_training.pdf' },
    { id: '4', type: 'Proof of Address', status: 'missing' },
  ]);
  ...
  const handleUpload = (id: string, fileName: string = 'uploaded_doc.pdf') => {
    ...
    setDocs(docs.map(doc => 
      doc.id === id 
        ? { ...doc, status: 'valid', fileName, expiryDate: '2027-01-01' } 
        : doc
    ));
    addNotification({
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto w-full space-y-8 pb-24">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-serif font-bold tracking-tight">Staff Profile</h1>
        <div className="flex gap-3">
          <a href="#compliance" className="hidden sm:flex items-center gap-2 bg-rr-bg text-rr-text/70 px-4 py-2 rounded-xl font-semibold text-sm hover:bg-rr-accent/10 transition-colors border border-rr-accent/20">
      ...
      <div className="bg-white rounded-3xl p-6 border border-rr-accent/20 flex flex-col sm:flex-row items-center gap-6 shadow-sm">
        <div className="relative group">
          <div className="w-24 h-24 rounded-full bg-rr-bg border-2 border-rr-accent/20 overflow-hidden">
            <img 
        ...
        <div className="text-center sm:text-left">
          ...
          <p className="text-rr-text/60 font-medium">Senior Healthcare Assistant</p>
          <div className="mt-2 flex items-center justify-center sm:justify-start gap-2">
            <span className="px-2 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold uppercase tracking-wider rounded-md">Active</span>
            <span className="px-2 py-0.5 bg-rr-bg text-rr-text/60 text-[10px] font-bold uppercase tracking-wider rounded-md">ID: RR-8921</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-serif font-semibold text-xl">Personal Details</h3>
          </div>
          <div className="bg-white rounded-2xl p-6 border border-rr-accent/20 space-y-4 shadow-sm">
            ...
            <InputField label="Date of Birth" value={personalInfo.dob} icon={<Calendar className="w-4 h-4" />} />
            <InputField label="Nationality" value={personalInfo.nationality} icon={<Globe className="w-4 h-4" />} />
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 border border-rr-accent/20 space-y-4 shadow-sm">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-rr-text/40 uppercase tracking-wider">Home Address</label>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-serif font-semibold text-xl">Availability & Preferences</h3>
        </div>
        <div className="bg-white rounded-2xl p-6 border border-rr-accent/20 space-y-6 shadow-sm">
          <div className="space-y-2">
            <label className="text-xs font-bold text-rr-text/40 uppercase tracking-wider">Available Days</label>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                    availability.days.includes(day)
                      ? 'bg-rr-text text-rr-bg border-rr-text'
                      : 'bg-white text-rr-text/60 border-rr-accent/20 hover:border-rr-text/40'
                  }`}
                >
                  {day}
                </button>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-rr-text/40 uppercase tracking-wider">Preferred Hours</label>
            <select
              value={availability.hours}
              onChange={(e) => setAvailability({...availability, hours: e.target.value})}
              className="w-full sm:w-auto px-4 py-2 bg-rr-bg/50 border border-rr-accent/20 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-rr-text/10"
            >
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-rr-text/40 uppercase tracking-wider">Preferred Roles</label>
            <div className="flex flex-wrap gap-2">
              {availableRoles.map(role => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                    availability.roles.includes(role)
                      ? 'bg-rr-accent text-white border-rr-accent'
                      : 'bg-white text-rr-text/60 border-rr-accent/20 hover:border-rr-text/40'
                  }`}
                >
                  {role}
      ...
      <section id="compliance" className="space-y-4 scroll-mt-24">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif font-semibold text-xl">Compliance Documents</h3>
          </div>
...
function ComplianceSummaryCard({ label, status, icon, color }: { label: string; status: string; icon: ReactNode; color: 'green' | 'orange' | 'red' }) {
  const colorClasses = {
  ...
  return (
    <div className={`p-4 rounded-2xl border ${colorClasses[color]} flex items-center gap-4 shadow-sm`}>
