...
import { User, Mail, Phone, MapPin, Upload, CheckCircle2, AlertCircle, Calendar, Globe, Trash2, FileText, ShieldCheck, Eye, RefreshCw, Shield } from 'lucide-react';
...
export default function AdminProfile() {
  ...
  const [contactInfo, setContactInfo] = useState({
    email: 'admin@renewrehoboth.com',
    phone: '+44 800 123 4567',
    address: 'Renew Rehoboth HQ, London',
  });

  const [docs, setDocs] = useState<ComplianceDoc[]>([
    { id: '1', type: 'Admin Training Certificate', status: 'valid', expiryDate: '2026-08-15', fileName: 'admin_training.pdf' },
    { id: '2', type: 'Data Protection Policy Acknowledgment', status: 'valid', expiryDate: '2025-01-10', fileName: 'gdpr_ack.pdf' },
    { id: '3', type: 'System Access Agreement', status: 'missing' },
  ]);
  ...
  const handleRequestRenewal = (type: string) => {
    addNotification({
      ...
      message: `A renewal request for ${type} has been sent to HR.`,
      type: 'success'
    });
  };
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto w-full space-y-8 pb-24">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-serif font-bold tracking-tight">Admin Profile</h1>
      ...
      <div className="bg-white rounded-3xl p-6 border border-rr-accent/20 flex flex-col sm:flex-row items-center gap-6 shadow-sm">
        <div className="relative group">
          <div className="w-24 h-24 rounded-full bg-rr-bg border-2 border-rr-accent/20 overflow-hidden flex items-center justify-center">
            <Shield className="w-12 h-12 text-rr-text/40" />
          </div>
        ...
        <div className="text-center sm:text-left">
          ...
          <p className="text-rr-text/60 font-medium">{personalInfo.role}</p>
          <div className="mt-2 flex items-center justify-center sm:justify-start gap-2">
            <span className="px-2 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold uppercase tracking-wider rounded-md">Super Admin</span>
            <span className="px-2 py-0.5 bg-rr-bg text-rr-text/60 text-[10px] font-bold uppercase tracking-wider rounded-md">ID: ADM-001</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="font-serif font-semibold text-xl">Admin Details</h3>
          </div>
          <div className="bg-white rounded-2xl p-6 border border-rr-accent/20 space-y-4 shadow-sm">
            ...
            <InputField label="Role" value={personalInfo.role} icon={<Shield className="w-4 h-4" />} />
            <InputField label="Department" value={personalInfo.department} />
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-6 border border-rr-accent/20 space-y-4 shadow-sm">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-rr-text/40 uppercase tracking-wider">Office Address</label>
      ...
      <section id="compliance" className="space-y-4 scroll-mt-24">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif font-semibold text-xl">Admin Documents</h3>
          </div>
